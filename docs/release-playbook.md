...
This is the canonical publish procedure for `pi-handoff`.

## Scope

- Release channel: **latest** only (stable tags `vX.Y.Z`).
- Publish target: npm package **`pi-handoff`**.
...
6. **Post-release verification**
   - `npm view pi-handoff version dist-tags --json`
   - `npm view` may lag due cache. Verify with registry data:
     ```bash
     curl -s https://registry.npmjs.org/pi-handoff \
...
- If a bad version is published:
  1. Deprecate bad version:
     - `npm deprecate pi-handoff@X.Y.Z "broken release; upgrade to >=X.Y.Z+1"`
